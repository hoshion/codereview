...
class UserService {
  ...
  async register (email, password) {
    if (this.users.exists(email)) throw ApiError.BadRequest('Пользователь с таким почтовым адресом уже существует!')
  ...
  async login (email, password) {
    if (!this.users.exists(email)) throw ApiError.BadRequest('Пользователь не был найден!')
